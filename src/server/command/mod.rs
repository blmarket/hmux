...
mod tests {
    ...
    #[test]
    fn run_shell_background_returns_before_completion() {
        let marker =
            std::env::temp_dir().join(format!("hmux-run-shell-background-{}", std::process::id()));
        let _ = std::fs::remove_file(&marker);
        let script = format!("sleep 0.2; : > '{}'", marker.display());
        let st = state();

        let result = run_str(&st, &["run-shell", "-b", &script]);

        assert_eq!(result.exit, 0);
        assert!(!marker.exists(), "background shell completed before return");
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
        while !marker.exists() && std::time::Instant::now() < deadline {
            std::thread::sleep(std::time::Duration::from_millis(10));
        }
        assert!(marker.exists(), "background shell did not complete");
        let _ = std::fs::remove_file(marker);
    }
    ...
    #[test]
    fn pane_current_path_follows_child_cd() {
        // End-to-end through the command/format path: a pane whose shell has
        // `cd`'d reports the new directory via #{pane_current_path}, read live
        // from the child. Before the fix this was hardcoded to the server's cwd
        // (std::env::current_dir()), so it never reflected the pane's real dir.
        let target = std::fs::canonicalize(std::env::temp_dir())
            .expect("temporary directory")
            .to_string_lossy()
            .into_owned();
        let mut inner = ServerState::empty();
        inner
            .create_session(
                "0",
                PaneSpec::Command(vec![
                    "/bin/sh".into(),
                    "-c".into(),
                    "cd \"${TMPDIR:-/tmp}\" && sleep 30".into(),
                ]),
            )
            .expect("create session with command pane");
        let st = Arc::new(Mutex::new(inner));
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
        loop {
            let r = run_str(
                &st,
                &["display-message", "-t", "0", "-p", "#{pane_current_path}"],
            );
            assert_eq!(r.exit, 0);
            if r.stdout == format!("{target}\n") {
                break;
            }
            assert!(
    ...
    #[test]
    fn pane_current_command_reports_foreground_program() {
        let mut inner = ServerState::empty();
        inner
            .create_session("0", PaneSpec::Command(vec!["/bin/sh".into()]))
            .expect("create session with shell pane");
        let st = Arc::new(Mutex::new(inner));
        ...
        loop {
            ...
            assert!(
                std::time::Instant::now() < deadline,
                "#{{pane_current_command}} never reported sh; got {:?}",
                r.stdout
            );
            std::thread::sleep(std::time::Duration::from_millis(20));
        }
    ...
    #[test]
    fn inherited_automatic_rename_format_tracks_foreground_program() {
        let mut inner = ServerState::empty();
        inner
            .create_session("0", PaneSpec::Command(vec!["/bin/sh".into()]))
            .expect("create session with shell pane");
        ...
        assert_eq!(
            run_str(&st, &["send-keys", "-l", "-t", "0", "sleep 30"]).exit,
            0
        );
        ...
        loop {
            ...
            assert_eq!(r.exit, 0);
            if r.stdout == "sleep:sleep\n" {
                break;
            }
            assert!(
                std::time::Instant::now() < deadline,
                "default automatic rename never followed sleep; got {:?}",
                r.stdout
            );
            std::thread::sleep(std::time::Duration::from_millis(20));
        }
    ...
    /// Read `#{pane_pipe}` for the target pane via the full format path.
    fn pane_pipe(st: &Arc<Mutex<ServerState>>, target: &str) -> String {
        run_str(st, &["display-message", "-p", "-t", target, "#{pane_pipe}"]).stdout
    }

    #[test]
    fn pipe_pane_opens_and_closes_flag() {
        let st = state();
        // Opening a pipe with a command sets #{pane_pipe}; a bare pipe-pane closes it.
        assert_eq!(run_str(&st, &["pipe-pane", "-t", "0", "cat"]).exit, 0);
        assert_eq!(pane_pipe(&st, "0"), "1\n");
        assert_eq!(run_str(&st, &["pipe-pane", "-t", "0"]).exit, 0);
        assert_eq!(pane_pipe(&st, "0"), "0\n");
    }

    #[test]
    fn pipe_pane_dash_o_toggles() {
        let st = state();
        // -o with no pipe open opens one; repeating -o toggles it back off, then on.
        run_str(&st, &["pipe-pane", "-o", "-t", "0", "cat"]);
        assert_eq!(pane_pipe(&st, "0"), "1\n");
        run_str(&st, &["pipe-pane", "-o", "-t", "0", "cat"]);
        assert_eq!(pane_pipe(&st, "0"), "0\n");
        run_str(&st, &["pipe-pane", "-o", "-t", "0", "cat"]);
    ...
    #[test]
    fn pipe_pane_replace_stays_open() {
        let st = state();
        // A second command without -o replaces the pipe but leaves it open.
        run_str(&st, &["pipe-pane", "-t", "0", "cat"]);
        run_str(&st, &["pipe-pane", "-t", "0", "cat"]);
        assert_eq!(pane_pipe(&st, "0"), "1\n");
    }

    #[test]
    fn pipe_pane_is_per_pane() {
        let st = state();
        run_str(&st, &["split-window", "-t", "0:0"]); // panes 0,1
        run_str(&st, &["pipe-pane", "-t", "0:0.1", "cat"]);
        let lp = run_str(
            &st,
            &[
                "list-panes",
    ...
    #[test]
    fn wait_for_in_command_list_does_not_hold_server_state_lock() {
        let st = state();
        let waiter_state = Arc::clone(&st);
        let waiter = std::thread::spawn(move || {
            run_str(
                &waiter_state,
                &["wait-for", "ready", ";", "display-message", "-p", "done"],
            )
        });
        std::thread::sleep(std::time::Duration::from_millis(50));

        assert_eq!(run_str(&st, &["wait-for", "-S", "ready"]).exit, 0);
        let result = waiter.join().expect("wait-for command thread");
        assert_eq!(result.exit, 0);
        assert_eq!(result.stdout_data(), b"done\n");
    }
