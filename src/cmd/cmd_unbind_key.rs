...
pub(crate) static cmd_unbind_key_entry: cmd_entry = {
    cmd_entry {
        name: c"unbind-key",
        alias: Some(c"unbind"),
        args: args_parse_t {
...
unsafe fn cmd_unbind_key_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if args_has(args, 'a' as i32 as u_char) != 0 {
            if !keystr.is_null() {
                if quiet == 0 {
                    cmdq_error(item, c"key given with -a".as_ptr(), fmt_args![]);
                }
        ...
        if keystr.is_null() {
            if quiet == 0 {
                cmdq_error(item, c"missing key".as_ptr(), fmt_args![]);
            }
            return CMD_RETURN_ERROR;
        }
        ...
        {
            if quiet == 0 {
                cmdq_error(item, c"unknown key: %s".as_ptr(), fmt_args![keystr]);
            }
            return CMD_RETURN_ERROR;
        }
