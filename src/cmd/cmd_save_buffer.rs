...
use crate::paste::{paste_buffer_data, paste_get_name, paste_get_top};
pub use crate::types::*;
use ::std::ffi::CString;
...
pub(crate) static cmd_save_buffer_entry: cmd_entry = {
    cmd_entry {
        name: c"save-buffer",
        alias: Some(c"saveb"),
        args: args_parse_t {
...
pub(crate) static cmd_show_buffer_entry: cmd_entry = {
    cmd_entry {
        name: c"show-buffer",
        alias: Some(c"showb"),
        args: args_parse_t {
...
unsafe fn cmd_save_buffer_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        let bufdata = paste_buffer_data(&*pb);
