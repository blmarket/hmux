...
use crate::fmt_args;
use crate::format::{format_create, format_defaults_paste_buffer, format_expand, format_true};
use crate::sort::{sort_get_buffers, sort_order_from_string};
pub use crate::types::*;
use ::core::ffi::CStr;
...
pub(crate) static cmd_list_buffers_entry: cmd_entry = cmd_entry {
    name: c"list-buffers",
    alias: Some(c"lsb"),
    args: args_parse_t {
        template: c"F:f:O:r",
...
fn sorted_buffers(sort_crit: &mut sort_criteria_t) -> Vec<*mut paste_buffer> {
    unsafe { sort_get_buffers(sort_crit) }
}

unsafe fn cmd_list_buffers_exec(self_0: &cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        let args = cmd_get_args(self_0);

        let template = match option(args, b'F') {
            Some(template) => template.as_ptr(),
            None => LIST_BUFFERS_TEMPLATE.as_ptr(),
        };
