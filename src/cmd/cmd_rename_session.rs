use crate::arguments::args_string;
...
use crate::session::{session_find, session_name, session_rename};
...
pub(crate) static cmd_rename_session_entry: cmd_entry = {
    cmd_entry {
...
unsafe fn cmd_rename_session_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        let args: &args = cmd_get_args(self_0);
        let mut target: *mut cmd_find_state = cmdq_get_target(item);
        let mut s: *mut session = (*target).session();
        let tmp = format_single_from_target(
            item,
        ...
        let Some(newname) = clean_name(tmp.as_ptr(), 0 as ::core::ffi::c_int) else {
            return CMD_RETURN_NORMAL;
        };
        if strcmp(newname.as_ptr(), session_name(s)) == 0 as ::core::ffi::c_int {
            return CMD_RETURN_NORMAL;
        }
        if !session_find(newname.as_ptr()).is_null() {
            cmdq_error(
                ...
                fmt_args![newname.as_ptr()],
            );
            return CMD_RETURN_ERROR;
        }
        session_rename(s, newname);
