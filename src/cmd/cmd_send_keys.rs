...
use crate::key_bindings::{key_bindings_dispatch, key_bindings_get, key_bindings_get_table_ref};
use crate::options::options_get_number;
use crate::server::server_client_handle_key;
use crate::session::session_options;
use crate::style::colour_palette_clear;
...
use crate::window::window_pane_key;
...
pub(crate) static cmd_send_keys_entry: cmd_entry = {
    cmd_entry {
        name: c"send-keys",
        alias: Some(c"send"),
        args: args_parse_t {
...
pub(crate) static cmd_send_prefix_entry: cmd_entry = {
    cmd_entry {
        name: c"send-prefix",
        alias: None,
        args: args_parse_t {
...
unsafe fn cmd_send_keys_inject_key(
    ...
    unsafe {
        ...
        if args_has(args, 'K' as i32 as u_char) != 0 {
            ...
            server_client_handle_key(tc, event);
            return item;
        }
        wme = window_pane_current_mode(wp);
        let mode_table = if wme.is_null() {
            None
        } else {
            (*wme).mode().key_table(wme)
        };
        ...
        let table_ref = key_bindings_get_table_ref(mode_table.as_ptr(), 1 as ::core::ffi::c_int)
            .expect("mode key table creation requested");
        table = table_ref.as_ptr();
        bd = key_bindings_get(table, key & !KEYC_MASK_FLAGS);
        if !bd.is_null() {
            after =
                key_bindings_dispatch(bd, after, tc, ::core::ptr::null_mut::<key_event>(), target);
        }
        after
    }
}
...
unsafe fn cmd_send_keys_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if args_has(args, 'N' as i32 as u_char) != 0 {
            ...
            if let Some(cause) = cause.as_ref() {
                cmdq_error(item, c"repeat count %s".as_ptr(), fmt_args![cause.as_ptr()]);
                return CMD_RETURN_ERROR;
            }
            if !wme.is_null() && (args_has(args, 'X' as i32 as u_char) != 0 || count == 0 as u_int)
            {
                if !(*wme).mode().has_command() {
        ...
        if args_has(args, 'X' as i32 as u_char) != 0 {
            if wme.is_null() || !(*wme).mode().has_command() {
                cmdq_error(item, c"not in a mode".as_ptr(), fmt_args![]);
                return CMD_RETURN_ERROR;
            }
        ...
        if ::core::ptr::eq(cmd_get_entry(self_0), &cmd_send_prefix_entry) {
            if args_has(args, '2' as i32 as u_char) != 0 {
                key = options_get_number(session_options(s), c"prefix2".as_ptr()) as key_code;
            } else {
                key = options_get_number(session_options(s), c"prefix".as_ptr()) as key_code;
            }
