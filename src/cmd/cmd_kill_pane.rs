...
//!
//! The `-a` walk reads each pane's successor before the body has it, the way
//! the C's `TAILQ_FOREACH_SAFE` did, since `window_remove_pane` frees the pane
//! it is given. The list stays the window's own, walked through its links,
//! because `window_pane` is the crate's type and every other module reads the
//! same list.
//!
...
//! `test_coverage_cmd_kill_pane` reads and pins them through it, so they stay
...
use crate::server::{server_kill_pane, server_redraw_window, server_unzoom_window};
pub use crate::types::*;
use crate::window::{window_panes_first, window_panes_next, window_remove_pane};
use ::core::ffi::c_char;
...
pub(crate) static cmd_kill_pane_entry: cmd_entry = cmd_entry {
    name: c"kill-pane",
    alias: Some(c"killp"),
    args: args_parse_t {
        template: c"at:",
...
/// The panes of `w`, in window order, each one's successor read before the
/// walk hands it over so that removing it does not lose the rest of the list.
fn panes_of(w: *mut window) -> impl Iterator<Item = *mut window_pane> {
    let mut next = unsafe { window_panes_first(w) };
    ::core::iter::from_fn(move || unsafe {
        let current = next;
        (!current.is_null()).then(|| {
            next = window_panes_next(w, current);
            current
        })
    })
}

/// Lets `wp` go: off every client that was reading it, out of its window's
/// layout, and out of the window itself, which frees it.
unsafe fn remove(w: *mut window, wp: *mut window_pane) {
    unsafe {
...
unsafe fn cmd_kill_pane_exec(self_0: &cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if args_has(args, b'a') != 0 {
            ...
            for loopwp in panes_of(w).filter(|&loopwp| loopwp != wp) {
                remove(w, loopwp);
            }
        ...
        if wp.is_null() {
            cmdq_error(item, c"no active pane to kill".as_ptr(), fmt_args![]);
