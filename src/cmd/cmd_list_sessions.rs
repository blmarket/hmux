...
use crate::sort::{sort_get_sessions, sort_order_from_string};
pub use crate::types::*;
use ::core::ffi::CStr;
...
pub(crate) static cmd_list_sessions_entry: cmd_entry = cmd_entry {
...
fn sorted_sessions(sort_crit: &mut sort_criteria_t) -> Vec<*mut session> {
    unsafe { sort_get_sessions(sort_crit) }
}

unsafe fn cmd_list_sessions_exec(self_0: &cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        let args = cmd_get_args(self_0);

        let template = match option(args, b'F') {
            Some(template) => template.as_ptr(),
            None => LIST_SESSIONS_TEMPLATE.as_ptr(),
        };
        ...
        for (i, &s) in sorted_sessions(&mut sort_crit).iter().enumerate() {
            ...
            format_add(&mut ft, c"line", c"%u".as_ptr(), fmt_args![i as u_int]);
            format_defaults(&mut ft, null_mut(), s, null_mut(), null_mut());
