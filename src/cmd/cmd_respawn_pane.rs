...
pub(crate) static cmd_respawn_pane_entry: cmd_entry = {
    cmd_entry {
        name: c"respawn-pane",
...
unsafe fn cmd_respawn_pane_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if spawn_pane(&mut sc, &mut cause).is_null() {
            let cause = cause.unwrap();
            cmdq_error(
                item,
                c"respawn pane failed: %s".as_ptr(),
