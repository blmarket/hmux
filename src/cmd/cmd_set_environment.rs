...
use crate::environ::{environ_clear, environ_set, environ_t, environ_unset};
...
pub(crate) static cmd_set_environment_entry: cmd_entry = {
    cmd_entry {
...
unsafe fn cmd_set_environment_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if *name as ::core::ffi::c_int == '\0' as i32 {
            cmdq_error(item, c"empty variable name".as_ptr(), fmt_args![]);
            return CMD_RETURN_ERROR;
        }
        if !strchr(name, '=' as i32).is_null() {
            cmdq_error(item, c"variable name contains =".as_ptr(), fmt_args![]);
            return CMD_RETURN_ERROR;
        }
        ...
        if args_has(args, 'g' as i32 as u_char) != 0 {
        ...
        } else if (*target).session().is_null() {
            ...
            current_block = 16977168487392565609;
        } else {
        ...
        if current_block == 224731115979188411 {
            if args_has(args, 'u' as i32 as u_char) != 0 {
                if !value.is_null() {
                    cmdq_error(item, c"can't specify a value with -u".as_ptr(), fmt_args![]);
                    retval = CMD_RETURN_ERROR;
                } else {
                ...
                if !value.is_null() {
                    cmdq_error(item, c"can't specify a value with -r".as_ptr(), fmt_args![]);
                    retval = CMD_RETURN_ERROR;
                } else {
                    environ_clear(env, name);
                }
            } else if value.is_null() {
                cmdq_error(item, c"no value specified".as_ptr(), fmt_args![]);
                retval = CMD_RETURN_ERROR;
            } else if args_has(args, 'h' as i32 as u_char) != 0 {
                environ_set(env, name, ENVIRON_HIDDEN, c"%s".as_ptr(), fmt_args![value]);
            } else {
                environ_set(
                    env,
                    name,
