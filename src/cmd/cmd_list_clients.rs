...
use crate::sort::{sort_get_clients, sort_order_from_string};
pub use crate::types::*;
...
pub(crate) static cmd_list_clients_entry: cmd_entry = cmd_entry {
    name: c"list-clients",
    alias: Some(c"lsc"),
    args: args_parse_t {
        template: c"F:f:O:rt:",
...
fn sorted_clients(sort_crit: &mut sort_criteria_t) -> Vec<*mut client> {
    unsafe { sort_get_clients(sort_crit) }
}

unsafe fn cmd_list_clients_exec(self_0: &cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        let template = match option(args, b'F') {
            ...
            None => LIST_CLIENTS_TEMPLATE.as_ptr(),
        };
        ...
        for (i, &c) in sorted_clients(&mut sort_crit).iter().enumerate() {
            ...
            format_defaults(&mut ft, c, null_mut(), null_mut(), null_mut());
