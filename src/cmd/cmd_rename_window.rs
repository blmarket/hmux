...
pub use crate::types::*;
use crate::window::window_set_name;
...
pub(crate) static cmd_rename_window_entry: cmd_entry = {
    cmd_entry {
...
unsafe fn cmd_rename_window_exec(mut self_0: &cmd, mut item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        let args: &args = cmd_get_args(self_0);
        let mut target: *mut cmd_find_state = cmdq_get_target(item);
        let mut wl: *mut winlink = (*target).winlink();
        let name = format_single_from_target(
            item,
