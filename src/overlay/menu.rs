...
use crate::cmd::{
    CmdqItemWeak, cmdq_append, cmdq_continue, cmdq_get_error, cmdq_get_event,
    cmdq_item_weak_from_ptr, cmdq_new_state,
};
...
use crate::server::server_client_overlay_range;
...
pub unsafe fn menu_add_item(
    ...
    unsafe {
        ...
        line = !item.is_some_and(|item| item.name.is_some_and(|name| !name.is_empty()))
            as ::core::ffi::c_int;
        ...
        let item_name = item.name.expect("a menu line has a name").as_ptr();
        let expanded = if !fs.is_null() {
            format_single_from_state(qitem, CStr::from_ptr(item_name), c, fs)
        } else {
            format_single(
                qitem,
                CStr::from_ptr(item_name),
                c,
        ...
        let trimmed = format_trim_right(CStr::from_ptr(s).to_bytes(), max_width);
        let name = if !key.is_null() {
            xasprintf(
                c"%s%s#[default] #[align=right](%s)".as_ptr(),
                fmt_args![trimmed.as_ptr(), suffix, key],
            )
        } else {
            xasprintf(c"%s%s".as_ptr(), fmt_args![trimmed.as_ptr(), suffix])
        };
        cmd = item.command.map_or(::core::ptr::null(), CStr::as_ptr);
        let command = if cmd.is_null() {
            None
        } else if !fs.is_null() {
            Some(format_single_from_state(qitem, CStr::from_ptr(cmd), c, fs))
        } else {
            Some(format_single(
                qitem,
                CStr::from_ptr(cmd),
        ...
        width = format_width(name.as_bytes());
        if *name.as_ptr() as ::core::ffi::c_int == '-' as i32 {
...
pub unsafe fn menu_create(mut title: *const ::core::ffi::c_char) -> Box<menu> {
    unsafe {
        let title = CStr::from_ptr(title);
        Box::new(menu {
            title: Some(title.to_owned()),
            items: Vec::new(),
            width: format_width(title.to_bytes()),
        })
    }
}
/// The menu's own screen, and where its cursor stands on the terminal.
pub unsafe fn menu_mode_cb(
    _c: *mut client,
    mut data: *mut menu_data,
) -> (*mut screen, u_int, u_int) {
    unsafe {
        let mut md: *mut menu_data = data;
        let cx = (*md).px.wrapping_add(2 as u_int);
        let cy = if (*md).choice == -(1 as ::core::ffi::c_int) {
            (*md).py
        } else {
            (*md)
                .py
...
unsafe fn menu_reapply_styles(mut md: *mut menu_data, mut c: *mut client) {
    unsafe {
        ...
        if let Some(style) = (*md).style.as_deref() {
            ...
            if style_parse(&mut sytmp, &(*md).style_gc, style.to_bytes()) == 0 as ::core::ffi::c_int
            {
        ...
        if let Some(style) = (*md).selected_style.as_deref() {
            style_set(&mut sytmp, &grid_default_cell);
            if style_parse(&mut sytmp, &(*md).selected_style_gc, style.to_bytes())
                == 0 as ::core::ffi::c_int
            {
        ...
        if let Some(style) = (*md).border_style.as_deref() {
            style_set(&mut sytmp, &grid_default_cell);
            if style_parse(&mut sytmp, &(*md).border_style_gc, style.to_bytes())
                == 0 as ::core::ffi::c_int
            {
...
pub unsafe fn menu_key_cb(
    ...
    unsafe {
        ...
        let mut i: u_int = 0;
        let mut count: ::core::ffi::c_int = (*menu).items.len() as ::core::ffi::c_int;
        let mut old: ::core::ffi::c_int = (*md).choice;
        let mut name: *const ::core::ffi::c_char = ::core::ptr::null::<::core::ffi::c_char>();
        let mut item: *const menu_entry = ::core::ptr::null::<menu_entry>();
        let mut error = None;
                        ...
                        << 32 as ::core::ffi::c_int
        {
            ...
            match current_block {
                3989530248692967800 => {}
                _ => {
                    (*md).choice = (*m).y.wrapping_sub((*md).py.wrapping_add(1 as u_int))
                        as ::core::ffi::c_int;
                    if (*md).choice != old {
                        (*c).flags |= CLIENT_REDRAWOVERLAY as uint64_t;
                    }
            ...
            loop {
                ...
                name = cstr_ptr(&(&(*menu).items)[i as usize].name);
                    ...
                    8589934618 | 8589934619 | 107 => {
                        ...
                        match current_block {
                            9508525478627385121 => {
                                ...
                                name = cstr_ptr(&(&(*menu).items)[(*md).choice as usize].name);
                            ...
                            6983216613468852725 => {
                                if (*md).choice > count - 6 as ::core::ffi::c_int {
                                    ...
                                    while i > 0 as u_int {
                                        (*md).choice += 1;
                                        name =
                                            cstr_ptr(&(&(*menu).items)[(*md).choice as usize].name);
                        ...
                        match current_block {
                            18425699056680496821 => return 0 as ::core::ffi::c_int,
                            _ => {
                                if old == -(1 as ::core::ffi::c_int) {
                                    old = 0 as ::core::ffi::c_int;
                                }
        ...
        event = match (*md).item.as_ref().and_then(CmdqItemWeak::upgrade) {
            Some(item) => cmdq_get_event(item.as_ptr()),
            None => ::core::ptr::null_mut::<key_event>(),
        };
...
pub(crate) unsafe fn menu_resize_cb(mut c: *mut client, mut data: *mut menu_data) {
    unsafe {
        let mut md: *mut menu_data = data;
        let mut nx: u_int = 0;
        let mut ny: u_int = 0;
        ...
        nx = (*md).px;
        ny = (*md).py;
        w = (*md).menu.width.wrapping_add(4 as u_int);
        h = ((*md).menu.items.len() as u_int).wrapping_add(2 as u_int);
...
pub unsafe fn menu_prepare(
    ...
    unsafe {
        ...
        if lines as ::core::ffi::c_int == BOX_LINES_DEFAULT as ::core::ffi::c_int {
            lines = options_get_number(o, c"menu-border-lines".as_ptr()) as box_lines;
        }
        let menu_ptr = &raw mut *menu;
        let mut md_box = Box::new(menu_data {
        ...
        let md = &raw mut *md_box;
        if !style.is_null() {
            (*md).style = Some(CStr::from_ptr(style).to_owned());
        }
        if !selected_style.is_null() {
            (*md).selected_style = Some(CStr::from_ptr(selected_style).to_owned());
        }
        if !border_style.is_null() {
            (*md).border_style = Some(CStr::from_ptr(border_style).to_owned());
        }
        if !fs.is_null() {
        ...
        (*md).choice = -(1 as ::core::ffi::c_int);
        if (*md).flags & MENU_NOMOUSE != 0 {
            if starting_choice >= (*menu_ptr).items.len() as ::core::ffi::c_int {
                starting_choice = ((*menu_ptr).items.len() as u_int).wrapping_sub(1 as u_int)
                    as ::core::ffi::c_int;
                choice = starting_choice + 1 as ::core::ffi::c_int;
                loop {
                    name = cstr_ptr(
                        &(&(*menu_ptr).items)[(choice - 1 as ::core::ffi::c_int) as usize].name,
                    );
                    if !name.is_null() && *name as ::core::ffi::c_int != '-' as i32 {
                        ...
                        if choice == 0 as ::core::ffi::c_int {
                            choice = (*menu_ptr).items.len() as ::core::ffi::c_int;
                        }
                ...
                loop {
                    name = cstr_ptr(&(&(*menu_ptr).items)[choice as usize].name);
                    if !name.is_null() && *name as ::core::ffi::c_int != '-' as i32 {
                        (*md).choice = choice;
                        break;
                    } else {
                        choice += 1;
                        if choice == (*menu_ptr).items.len() as ::core::ffi::c_int {
...
pub unsafe fn menu_display(
    ...
    unsafe {
        ...
        server_client_set_overlay(c, 0 as u_int, Overlay::Menu, OverlayState::Menu(md));
        0 as ::core::ffi::c_int
    }
}
