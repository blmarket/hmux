...
impl Grid {
    ...
    fn write(&mut self, px: u_int, py: u_int, s: &str) {
        ...
        grid_view_set_cells(&mut *self, px, py, &gc, s.as_bytes());
    }

    /// The text of one screen line.
    fn text(&self, py: u_int) -> String {
        {
            let p = grid_view_string_cells(&*self, 0, py, 1000);
    ...
    fn history_text(&self, py: u_int) -> String {
        {
            ...
            let p = grid_view_string_cells(&*self, 0, py.wrapping_sub(hsize), 1000);
    ...
    fn screen(&self) -> Vec<String> {
        (0..self.sy).map(|py| self.text(py)).collect()
    }
}
...
fn one_cell_and_one_padding_cell_can_be_set_through_the_view() {
    let _guard = globals();
    let mut g = with_history(10, 2, 2);
    let mut gc = { grid_default_cell };
    gc.data.data[0] = b'z';
    {
        grid_view_set_cell(&mut *g, 0, 0, &gc);
...
fn clearing_the_view_leaves_the_history_alone() {
    ...
    g.write(0, 0, "abc");
    g.write(0, 1, "def");
    grid_view_clear(&mut *g, 0, 0, 10, 1, 8);
...
fn clearing_the_history_scrolls_the_used_lines_into_it() {
    ...
    g.write(0, 0, "one");
    g.write(0, 1, "two");
    grid_view_clear_history(&mut *g, 8);
...
fn a_full_region_scrolls_into_the_history() {
    ...
    g.write(0, 0, "one");
    g.write(0, 1, "two");
    grid_view_scroll_region_up(&mut *g, 0, 2, 8);
    assert_eq!(g.hsize, 1);
    assert_eq!(g.history_text(0), "one");
...
fn part_of_the_screen_scrolls_into_the_history_as_a_region() {
    ...
    g.write(0, 1, "two");
    g.write(0, 2, "three");
    grid_view_scroll_region_up(&mut *g, 0, 1, 8);
    assert_eq!(g.hsize, 1);
    assert_eq!(g.history_text(0), "one");
...
fn a_region_scrolls_down_by_moving_its_lines() {
    let _guard = globals();
    let mut g = with_history(10, 3, 1);
    g.write(0, 0, "one");
    g.write(0, 1, "two");
    g.write(0, 2, "three");
    grid_view_scroll_region_down(&mut *g, 0, 2, 8);
...
fn lines_are_inserted_by_pushing_the_ones_below_them_down() {
    ...
    g.write(0, 1, "two");
    g.write(0, 2, "three");
    grid_view_insert_lines(&mut *g, 1, 1, 8);
    assert_eq!(g.screen(), ["one", "", "two"]);
}
...
fn inserting_more_lines_than_the_region_holds_clears_what_is_left() {
    ...
    grid_view_insert_lines_region(&mut *g, 2, 1, 2, 8);
...
fn lines_are_deleted_by_pulling_the_ones_below_them_up() {
    ...
    g.write(0, 1, "two");
    g.write(0, 2, "three");
    grid_view_delete_lines(&mut *g, 0, 1, 8);
    assert_eq!(g.screen(), ["two", "three", ""]);
}
...
fn lines_are_deleted_inside_a_region_without_touching_the_rest() {
    ...
    grid_view_delete_lines_region(&mut *g, 2, 0, 1, 8);
...
fn cells_are_inserted_by_pushing_the_rest_of_the_line_along() {
    let _guard = globals();
    let mut g = with_history(10, 2, 1);
    g.write(0, 0, "abcdef");
    grid_view_insert_cells(&mut *g, 2, 0, 2, 8);
    assert_eq!(
        g.text(0),
        "ab  cdef  ",
...
fn inserting_cells_at_the_end_of_a_line_only_clears_the_last_one() {
    ...
    grid_view_insert_cells(&mut *g, 9, 0, 2, 8);
...
fn cells_are_deleted_by_pulling_the_rest_of_the_line_back() {
    ...
    let mut g = with_history(10, 2, 1);
    g.write(0, 0, "abcdef");
    grid_view_delete_cells(&mut *g, 1, 0, 2, 8);
    assert_eq!(g.text(0), "adef    ");
    assert_eq!(
        {
            grid_peek_line(&*g, g.hsize)
