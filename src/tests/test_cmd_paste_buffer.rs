...
fn the_separator_is_s_before_r_before_a_carriage_return() {
    let _guard = globals();
    {
        assert_eq!(separator(&*Args::parse(c"paste-buffer").borrow()), c"\r");
        assert_eq!(separator(&*Args::parse(c"paste-buffer -r").borrow()), c"\n");
        assert_eq!(
            separator(&*Args::parse(c"paste-buffer -s XY").borrow()),
            c"XY"
        );
        assert_eq!(
            separator(&*Args::parse(c"paste-buffer -r -s XY").borrow()),
            c"XY",
...
fn buffer_bytes_is_what_the_store_holds_newlines_and_all() {
    let _guard = globals();
    let store = Paste::new();
    store.add(c"lines", "ab\ncd\n");
    store.add(c"one", "x");
    assert_eq!(
        with_paste_buffers(|buffers| buffers.get(c"lines").map(|buffer| buffer.data.to_vec())),
        Some(b"ab\ncd\n".to_vec())
    );
    assert_eq!(
        with_paste_buffers(|buffers| buffers.get(c"one").map(|buffer| buffer.data.to_vec())),
        Some(b"x".to_vec())
    );
}
...
fn a_run_of_newlines_sends_a_separator_for_each_of_the_empty_lines() {
    ...
    unsafe {
        ...
        assert_eq!(
            it.with_command(|command, item| run(command, item)),
            CMD_RETURN_NORMAL
        );
