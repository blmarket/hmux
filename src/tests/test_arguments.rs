...
/// An owned array of command parser values.
...
fn the_values_of_a_flag_come_back_in_order() {
    {
        ...
        args_set(&mut args, b'a', string_value(c"one"), 0);
        args_set(&mut args, b'a', string_value(c"two"), 0);
        let values = args.argument_flag_values(b'a');
...
fn a_number_argument_is_read_from_the_last_value_of_a_flag() {
    {
        ...
        args_set(&mut args, b'a', string_value(c"3"), 0);
        assert_eq!(args_strtonum(&args, b'a', 0, 10, &mut cause), 3);
        assert!(cause.is_none());

        args_set(&mut args, b'a', string_value(c"11"), 0);
        assert_eq!(args_strtonum(&args, b'a', 0, 10, &mut cause), 0);
...
fn a_number_argument_can_be_expanded_first() {
    ...
    unsafe {
        ...
        args_set(&mut args, b'a', string_value(c"#{e|+|:1,2}"), 0);
        assert_eq!(
            args_strtonum_and_expand(&args, b'a', 0, 10, &runner.item(), &mut cause),
            3
        );
        ...
        args_set(&mut args, b'a', string_value(c"x"), 0);
        assert_eq!(
            args_strtonum_and_expand(&args, b'a', 0, 10, &runner.item(), &mut cause),
            0
        );
...
fn a_percentage_is_taken_of_the_current_value() {
    let percentage = |value: &CStr| {
        let mut cause = None;
        let ll = args_string_percentage(value, 1, 100, 50, &mut cause);
        if cause.is_none() {
            Ok(ll)
        } else {
    ...
    {
        let mut cause = None;
        assert_eq!(args_string_percentage(c"100%", 0, 10, 50, &mut cause), 0);
        assert_eq!(cause.take().unwrap().to_string_lossy(), "too large");
    }
}
...
fn a_percentage_argument_comes_from_the_last_value_of_a_flag() {
    {
        ...
        args_set(&mut args, b'a', string_value(c"50%"), 0);
...
fn an_expanded_percentage_argument_comes_from_the_last_value_of_a_flag() {
    ...
    unsafe {
        ...
        args_set(&mut args, b'a', string_value(c"#{e|+|:20,30}%"), 0);
        assert_eq!(
            args_percentage_and_expand(&args, b'a', 0, 100, 50, &runner.item(), &mut cause),
            25
        );
...
fn an_expanded_percentage_is_taken_of_the_current_value() {
    ...
    let percentage = |value: &CStr| unsafe {
        let mut cause = None;
        let ll = args_string_percentage_and_expand(value, 1, 100, 50, &*item, &mut cause);
        if cause.is_none() {
            Ok(ll)
        } else {
            Err(cause.take().unwrap().to_string_lossy().into_owned())
    ...
    unsafe {
        let mut cause = None;
        assert_eq!(
            args_string_percentage_and_expand(c"100%", 0, 10, 50, &*item, &mut cause),
            0
        );
        assert_eq!(cause.take().unwrap().to_string_lossy(), "too large");
    }
...
impl Runner {
    ...
    fn item(&self) -> std::cell::Ref<'_, cmdq_item> {
        self.item.read()
    }

    /// Run the command with the arguments a command line parses to.
    fn with_args(&mut self, values: &mut Values) {
...
fn a_command_list_argument_is_prepared_as_it_is() {
    ...
    unsafe {
        ...
        assert_eq!(
            (same.as_ref().unwrap()).print(0).to_string_lossy(),
            "display-message hello"
        );
        ...
        assert_ne!(copy, same);
        assert_eq!(
            (copy.as_ref().unwrap()).print(0).to_string_lossy(),
            "display-message hello"
        );
        drop(copy);
    }
...
fn a_prepared_command_can_be_expanded_first() {
    ...
    let mut values = Values::words(&[c"cmd"]);
    unsafe {
        (runner.item.item()).set_target_client(Some(&runner.client));
        runner.with_args(&mut values);
        let source_list = runner.cmdlist.clone();
        let command = source_list.command(0).expect("the prepared command");
        let state = cmd_make_commands_prepare(
            ...
            Some(c"display-message #{e|+|:1,2}"),
            1,
            |cmd| crate::format::format_single_from_target(&runner.item(), cmd),
        );
        ...
        assert!(
            state
                ...
                .is_some_and(|item| item.points_to(&*runner.item()))
        );
...
#[test]
fn command_preparation_accepts_a_consuming_expander_in_safe_code() {
    let _guard = exclusive();
    let mut runner = Runner::new();
    let mut values = Values::words(&[c"cmd", c"display-message %1"]);
    runner.with_args(&mut values);
    ...
    assert_eq!(state.cmd.as_deref(), Some(c"display-message expanded"));
    assert_eq!(state.pi.file(), Some(c"args.conf"));
    assert_eq!(state.pi.line, 7);
}
