...
fn utf8_isvalid(input: &CStr) -> c_int {
    RustUtf8VisModel.is_valid(input) as c_int
}

fn utf8_sanitize(input: &CStr) -> CString {
    RustUtf8VisModel.sanitize(input)
}

fn utf8_cstrwidth(input: &CStr) -> u32 {
    RustUtf8VisModel.width(input)
}

fn utf8_padcstr(input: &CStr, width: u32) -> CString {
    RustUtf8VisModel.pad_right(input, width)
}
...
fn utf8_isvalid_accepts_printable_ascii_and_valid_multibyte() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_isvalid(c" "), 1, "space 0x20 is printable");
        assert_eq!(utf8_isvalid(c"~"), 1, "tilde 0x7E is printable");
        assert_eq!(utf8_isvalid(c"a b~c"), 1);
        // valid 2-, 3- and 4-byte sequences are accepted
        assert_eq!(utf8_isvalid(c"a\xc3\xa9b"), 1, "é U+00E9");
        assert_eq!(utf8_isvalid(c"\xe2\x82\xac"), 1, "€ U+20AC");
        assert_eq!(utf8_isvalid(c"\xf0\x9f\x98\x80"), 1, "😀 U+1F600");
        // string that is only valid multibyte
        assert_eq!(utf8_isvalid(c"\xc3\xa9\xe2\x82\xac\xf0\x9f\x98\x80"), 1);
    }
}
...
fn utf8_isvalid_rejects_controls_and_broken_utf8() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_isvalid(c"a\x7f"), 0, "DEL 0x7F invalid");
        assert_eq!(utf8_isvalid(c"a\x01b"), 0, "SOH 0x01 invalid");
        assert_eq!(utf8_isvalid(c"\x1f"), 0, "0x1F invalid");
        assert_eq!(utf8_isvalid(c"\t"), 0, "tab invalid");
        assert_eq!(utf8_isvalid(c"\n"), 0, "newline invalid");
        // lone continuation / illegal starters
        assert_eq!(utf8_isvalid(c"\x80"), 0);
        assert_eq!(utf8_isvalid(c"\xff"), 0);
        assert_eq!(utf8_isvalid(c"\xfe"), 0);
        // truncated sequences – leading byte without enough continuations
        assert_eq!(utf8_isvalid(c"\xc3"), 0);
        assert_eq!(utf8_isvalid(c"\xe2\x82"), 0);
        assert_eq!(utf8_isvalid(c"\xf0\x9f\x98"), 0);
        // bad continuation byte
        assert_eq!(utf8_isvalid(c"\xc3\x28"), 0);
        assert_eq!(utf8_isvalid(c"\xe2\x28\xa1"), 0);
        // overlong / surrogate range that utf8_take refuses
        assert_eq!(utf8_isvalid(c"\xc0\x80"), 0);
        assert_eq!(utf8_isvalid(c"\xed\xa0\x80"), 0);
    }
}
...
fn utf8_sanitize_keeps_printable_ascii_replaces_controls() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_sanitize(c"a\x01b").as_bytes(), b"a_b");
        assert_eq!(utf8_sanitize(c"a\x7f").as_bytes(), b"a_");
        assert_eq!(utf8_sanitize(c"a\x7fb").as_bytes(), b"a_b");
        assert_eq!(utf8_sanitize(c"a\tb").as_bytes(), b"a_b");
        assert_eq!(utf8_sanitize(c"a\nb").as_bytes(), b"a_b");
        assert_eq!(utf8_sanitize(c"a\x01b\x7f").as_bytes(), b"a_b_");
        // lone invalid byte replaced
        assert_eq!(utf8_sanitize(c"\xff").as_bytes(), b"_");
        assert_eq!(utf8_sanitize(c"\x80").as_bytes(), b"_");
        // truncated leading byte replaced
        assert_eq!(utf8_sanitize(c"\xc3").as_bytes(), b"_");
        assert_eq!(utf8_sanitize(c"a\xc3").as_bytes(), b"a_");
    }
}
...
fn utf8_sanitize_replaces_each_character_by_its_display_width() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_sanitize(c"\xc3\xa9").as_bytes(), b"_");
        assert_eq!(utf8_sanitize(c"\xe2\x82\xac").as_bytes(), b"_", "€ width 1");
        // width 2 emoji -> two underscores
        assert_eq!(
            utf8_sanitize(c"\xf0\x9f\x98\x80").as_bytes(),
            b"__",
            "😀 width 2"
        );
        assert_eq!(utf8_sanitize(c"a\xf0\x9f\x98\x80b").as_bytes(), b"a__b");
        // mixed widths - length equals sum of widths / 1 per ASCII
        assert_eq!(
            utf8_sanitize(c"a\xc3\xa9\xf0\x9f\x98\x80").as_bytes(),
            b"a___",
            "1 + 1 + 2"
        );
        // empty stays empty
...
fn utf8_cstrwidth_counts_display_width_skipping_controls() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_cstrwidth(c" "), 1);
        assert_eq!(utf8_cstrwidth(c"~"), 1);
        // multibyte widths
        assert_eq!(utf8_cstrwidth(c"a\xc3\xa9"), 2, "a + é");
        assert_eq!(
            utf8_cstrwidth(c"a\xc3\xa9\xf0\x9f\x98\x80"),
            4,
            "a(1)+é(1)+😀(2)"
        );
        // controls contribute 0 width
        assert_eq!(utf8_cstrwidth(c"a\x01b"), 2);
        assert_eq!(utf8_cstrwidth(c"a\x7f"), 1);
        assert_eq!(utf8_cstrwidth(c"a\x01b\x7f"), 2);
        // truncated leading byte contributes 0
        assert_eq!(utf8_cstrwidth(c"\xc3"), 0);
        assert_eq!(utf8_cstrwidth(c"\xff"), 0);
    }
}
...
fn width_cache_defaults_are_applied_after_update() {
    ...
    unsafe {
        ...
        assert_eq!(utf8_cstrwidth(c"\xe2\x98\x9d"), 2, "U+261D default 2");
        // 0x1F1E6 (🇦 regional indicator) is default 1
        assert_eq!(utf8_cstrwidth(c"\xf0\x9f\x87\xa6"), 1, "U+1F1E6 default 1");
        // 0x1FAF8 is also in the table as 2
        assert_eq!(utf8_cstrwidth(c"\xf0\x9f\xab\xb8"), 2, "U+1FAF8 default 2");

        // sanitize reflects the same widths
        assert_eq!(utf8_sanitize(c"\xe2\x98\x9d").as_bytes(), b"__");
        assert_eq!(utf8_sanitize(c"\xf0\x9f\x87\xa6").as_bytes(), b"_");
...
fn width_cache_codepoint_widths_option_overrides_and_range() {
    ...
    unsafe {
        ...
        store.with_entry_mut(c"codepoint-widths", false, |entry| {
            RustOptionsEngine.array_set(entry.unwrap(), 0, Some(c"U+E9=2"), 0, &mut None)
        });
        utf8_update_width_cache(
            (global_options
                .as_ref()
        ...
        assert_eq!(utf8_cstrwidth(c"\xc3\xa9"), 2);
        assert_eq!(utf8_sanitize(c"\xc3\xa9").as_bytes(), b"__");
        ...
        assert_eq!(utf8_cstrwidth(c"\xc3\xa9"), 1);

        // range: U+E9..U+EB (é, ê, ë) to 2
        store.with_entry_mut(c"codepoint-widths", false, |entry| {
            RustOptionsEngine.array_set(entry.unwrap(), 0, Some(c"U+E9-U+EB=2"), 0, &mut None)
        });
        utf8_update_width_cache(
            (global_options
                .as_ref()
        ...
        assert_eq!(utf8_cstrwidth(c"\xc3\xa9"), 2, "U+E9");
        assert_eq!(utf8_cstrwidth(c"\xc3\xaa"), 2, "U+EA");
        assert_eq!(utf8_cstrwidth(c"\xc3\xab"), 2, "U+EB");
        assert_eq!(utf8_cstrwidth(c"\xc3\xac"), 1, "outside range stays 1");

        // another single: € (U+20AC) forced to 0
        store.with_entry_mut(c"codepoint-widths", false, |entry| {
            RustOptionsEngine.array_set(entry.unwrap(), 0, Some(c"U+20AC=0"), 0, &mut None)
        });
        utf8_update_width_cache(
            (global_options
                .as_ref()
        ...
        assert_eq!(utf8_cstrwidth(c"\xe2\x82\xac"), 0);
        assert_eq!(utf8_sanitize(c"\xe2\x82\xac").as_bytes(), b"");

        // invalid width 3 is rejected - width should be default again
        store.with_entry_mut(c"codepoint-widths", false, |entry| {
            RustOptionsEngine.array_set(entry.unwrap(), 0, Some(c"U+E9=3"), 0, &mut None)
        });
        utf8_update_width_cache(
            (global_options
                .as_ref()
        ...
        assert_eq!(utf8_cstrwidth(c"\xc3\xa9"), 1);
        assert_eq!(utf8_cstrwidth(c"\xe2\x82\xac"), 1);
        assert_eq!(utf8_cstrwidth(c"\xc3\xaa"), 1);
    }
}
