...
use crate::options::{
    ...
    options_parse_get, options_ptr, options_to_string,
};
...
impl format_tree {
    ...
    pub(crate) fn set_buffer(&mut self, pb: *mut paste_buffer) {
        self.pb_name = unsafe {
            pb.as_ref().map(|pb| {
                ::std::ffi::CStr::from_ptr(crate::paste::paste_buffer_name(
                    pb as *const paste_buffer as *mut paste_buffer,
                ))
                .to_owned()
            })
        };
...
unsafe fn format_cb_window_layout(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if !layout_root_ptr(&(*w).saved_layout_root).is_null() {
            return layout_dump(w, layout_root_ptr(&(*w).saved_layout_root));
        }
        layout_dump(w, layout_root_ptr(&(*w).layout_root))
    }
}
    ...
    unsafe {
        ...
        let Some(cmd) = cmd else {
            ...
            if command.as_bytes().is_empty() {
                let value = parse_window_name(cstr_ptr(&(*wp).shell));
                return Some(format_callback_copy(&value));
            }
            let value = parse_window_name(command.as_ptr());
            return Some(format_callback_copy(&value));
        };
...
unsafe fn format_cb_history_bytes(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        gd = screen_grid_ptr(&raw mut (*wp).base);
        i = 0 as u_int;
...
unsafe fn format_cb_pane_tabs(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        while i < (*screen_grid_ptr(&raw mut (*wp).base)).sx {
...
unsafe fn format_cb_pane_fg(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        tty_default_colours(&raw mut gc, wp);
...
unsafe fn format_cb_mouse_word(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if match cmd_mouse_at(wp, &raw mut m, 0 as ::core::ffi::c_int) {
            Some((at_x, at_y)) => {
                (x, y) = (at_x, at_y);
                false
            }
...
unsafe fn format_cb_buffer_name(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).buffer().is_null() {
            return Some(format_callback_copy(CStr::from_ptr(paste_buffer_name(
                (*ft).buffer(),
            ))));
        }
        None
...
unsafe fn format_cb_client_key_table(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).drawn_client().is_null() {
            return Some(format_callback_copy(CStr::from_ptr(key_table_name(
...
unsafe fn format_cb_client_prefix(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if !(*ft).drawn_client().is_null() {
            ...
            if strcmp(key_table_name((*(*ft).drawn_client()).keytable()), name)
                == 0 as ::core::ffi::c_int
            {
                return Some(format_callback_copy(c"0"));
            }
...
unsafe fn format_cb_client_uid(ft: &format_tree) -> Option<CString> {
    unsafe {
        let mut uid: uid_t = 0;
        if !(*ft).drawn_client().is_null() {
            uid = proc_get_peer_uid(peer_ptr(&(*(*ft).drawn_client()).peer));
...
unsafe fn format_cb_history_limit(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).pane().is_null() {
            return Some(format_printf(
                c"%u".as_ptr(),
                fmt_args![(*screen_grid_ptr(&raw mut (*(*ft).pane()).base)).hlimit],
            ));
        }
        None
    }
...
unsafe fn format_cb_history_size(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).pane().is_null() {
            return Some(format_printf(
                c"%u".as_ptr(),
                fmt_args![(*screen_grid_ptr(&raw mut (*(*ft).pane()).base)).hsize],
            ));
        }
        None
    }
...
unsafe fn format_cb_mouse_x(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if !wp.is_null()
            && match cmd_mouse_at(wp, &raw mut m, 0 as ::core::ffi::c_int) {
                Some((at_x, at_y)) => {
                    (x, _) = (at_x, at_y);
                    true
                }
...
unsafe fn format_cb_session_activity_flag(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if !(*ft).session().is_null() {
            wl = winlinks_first(&raw mut (*(*ft).session()).windows);
            if !wl.is_null() {
...
unsafe fn format_cb_session_group(ft: &format_tree) -> Option<CString> {
    unsafe {
        ...
        if !(*ft).session().is_null() && {
            ...
            return Some(format_callback_copy(CStr::from_ptr(session_group_name(sg))));
        }
        None
    }
}
...
unsafe fn format_cb_session_path(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).session().is_null() {
            return Some(format_callback_copy(CStr::from_ptr(session_cwd(
                (*ft).session(),
            ))));
        }
        None
...
unsafe fn format_cb_session_windows(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).session().is_null() {
            return Some(format_printf(
                c"%u".as_ptr(),
                fmt_args![winlink_count(&raw mut (*(*ft).session()).windows)],
            ));
        }
        None
    }
...
unsafe fn format_cb_socket_path(_ft: &format_tree) -> Option<CString> {
    unsafe { Some(format_callback_copy(CStr::from_ptr(socket_path))) }
}
unsafe fn format_cb_version(_ft: &format_tree) -> Option<CString> {
    unsafe { Some(format_callback_copy(CStr::from_ptr(getversion()))) }
}
...
unsafe fn format_cb_last_window_index(ft: &format_tree) -> Option<CString> {
    unsafe {
        let mut wl: *mut winlink = ::core::ptr::null_mut::<winlink>();
        if !(*ft).session().is_null() {
            wl = winlinks_last(&raw mut (*(*ft).session()).windows);
...
unsafe fn format_cb_window_bigger(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).drawn_client().is_null() {
            let (window_bigger, ..) = tty_window_offset(&raw mut (*(*ft).drawn_client()).tty);
            if window_bigger != 0 {
                return Some(format_callback_copy(c"1"));
            }
            return Some(format_callback_copy(c"0"));
...
unsafe fn format_cb_window_end_flag(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).winlink().is_null() {
            if (*ft).winlink() == winlinks_last(&raw mut (*(*(*ft).winlink()).session()).windows) {
                return Some(format_callback_copy(c"1"));
            }
            return Some(format_callback_copy(c"0"));
        }
    ...
    unsafe {
        ...
        while !s.is_null() {
            if session_group_contains(s).is_null()
                && !winlink_find_by_window(&raw mut (*s).windows, w).is_null()
            {
                n = n.wrapping_add(1);
            }
            s = sessions_after(s);
...
unsafe fn format_cb_window_name(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).window().is_null() {
            return Some(format_printf(
                c"%s".as_ptr(),
                fmt_args![cstr_ptr(&(*(*ft).window()).name)],
            ));
        }
        None
    }
...
unsafe fn format_cb_window_offset_x(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).drawn_client().is_null() {
            let (window_bigger, ox, ..) = tty_window_offset(&raw mut (*(*ft).drawn_client()).tty);
            if window_bigger != 0 {
...
unsafe fn format_cb_window_offset_y(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).drawn_client().is_null() {
            let (window_bigger, _ox, oy, ..) =
                tty_window_offset(&raw mut (*(*ft).drawn_client()).tty);
            if window_bigger != 0 {
...
unsafe fn format_cb_window_start_flag(ft: &format_tree) -> Option<CString> {
    unsafe {
        if !(*ft).winlink().is_null() {
            if (*ft).winlink() == winlinks_first(&raw mut (*(*(*ft).winlink()).session()).windows) {
                return Some(format_callback_copy(c"1"));
            }
            return Some(format_callback_copy(c"0"));
        }
...
pub unsafe fn format_merge(ft: &mut format_tree, from: &format_tree) {
    unsafe {
        ...
        for (key, value) in entries {
            format_add(&mut *ft, &key, c"%s".as_ptr(), fmt_args![value.as_ptr()]);
        }
    }
}
...
unsafe fn format_create_add_item(ft: &mut format_tree, mut item: *mut cmdq_item) {
    unsafe {
        ...
        cmdq_merge_formats(item, &mut *ft);
        ft.m = *m;
    }
}
pub unsafe fn format_create(
    ...
    unsafe {
        let mut ft = Box::new(format_tree {
            ...
            tree: format_entry_tree::new(),
        });
        ft.client_ref = client_ref_from_ptr(c);
        ft.set_item(item);
        ft.tag = tag as u_int;
        ft.flags = flags;
        if !item.is_null() {
...
pub unsafe fn format_add(
    ...
    unsafe {
        let fe = format_entry_for(&mut *ft, key);
        (*fe).cb = None;
        (*fe).time = 0 as time_t;
                ...
                if let Some(time) = ft.tree.get(wanted).map(|fe| fe.time) {
                    ...
                    if !modifiers & FORMAT_TIMESTRING != 0 {
                        ...
                        if envent.is_some_and(|envent| !environ_entry_value(envent).is_null()) {
                            let envent = envent.expect("the entry just looked at");
                            found = Some(CStr::from_ptr(environ_entry_value(envent)).to_owned());
...
unsafe fn format_choose(
    ...
    unsafe {
        ...
        cp = format_skip1(Some(&mut *es), s, c",");
        if cp.is_null() {
            return None;
        }
        if expand != 0 {
            ...
            let left = format_expand1(&mut *es, &left0);
            let right = format_expand1(&mut *es, &right0);
            Some((left, right))
        } else {
                ...
                && format_is_end(*cp.offset(2 as ::core::ffi::c_int as isize)) != 0
            {
                ...
                if format_is_end(*cp.offset(1 as ::core::ffi::c_int as isize)) != 0 {
                            ...
                            == '-' as i32
                    {
                        ...
                        let expanded = format_expand1(&mut *es, &value);
                        argv.push(expanded);
...
unsafe fn format_sub(fm: &format_modifier, text: &CStr, pattern: &CStr, with: &CStr) -> CString {
    unsafe {
        let text = text.as_ptr();
        let pattern = pattern.as_ptr();
        let with = with.as_ptr();
        let mut flags: ::core::ffi::c_int = REG_EXTENDED;
        if (fm.argv.len() as ::core::ffi::c_int) >= 3 as ::core::ffi::c_int
            && !strchr(fm.argv[2].as_ptr(), 'i' as i32).is_null()
        {
            flags |= REG_ICASE;
        }
        regsub(pattern, with, text, flags).unwrap_or_else(|| CStr::from_ptr(text).to_owned())
    }
}
...
unsafe fn format_bool_op_1(
    ...
    unsafe {
        ...
        let expanded = format_expand1(&mut *es, fmt);
...
unsafe fn format_bool_op_n(
    ...
    unsafe {
        ...
        {
            cp2 = format_skip1(Some(&mut *es), cp1, c",");
            ...
            let expanded = format_expand1(&mut *es, &raw);
            format_log1(
                &mut *es,
...
unsafe fn format_session_name(es: &mut format_expand_state, fmt: &CStr) -> Option<CString> {
    unsafe {
        let mut s: *mut session = ::core::ptr::null_mut::<session>();
        let name = format_expand1(&mut *es, fmt);
        s = sessions_first();
        while !s.is_null() {
            if strcmp(session_name(s), name.as_ptr()) == 0 as ::core::ffi::c_int {
...
unsafe fn format_loop_sessions(
    ...
    unsafe {
        ...
        let mut next = format_expand_state::default();
        let (all, active) = format_choose(&mut *es, fmt.as_ptr(), 0 as ::core::ffi::c_int)
...
unsafe fn format_window_name(es: &mut format_expand_state, fmt: &CStr) -> Option<CString> {
    unsafe {
        ...
        wl = winlinks_first(&raw mut (*(*ft).session()).windows);
        while !wl.is_null() {
            if strcmp(cstr_ptr(&(*(*wl).window()).name), name.as_ptr()) == 0 as ::core::ffi::c_int {
                return Some(c"1".to_owned());
            }
            wl = winlinks_after(wl);
        }
...
unsafe fn format_add_window_neighbor(
    ...
    unsafe {
        ...
        format_add(&mut *nft, &key, c"%u".as_ptr(), fmt_args![(*wl).idx]);
        ...
        o = options_first(options_ptr(&(*(*wl).window()).options));
        while !o.is_null() {
    ...
    unsafe {
        ...
        if current_block == 4495394744059808450 {
            ...
            match current_block {
                ...
                _ => {
                    let operands = format_choose(&mut *es, copy, 1 as ::core::ffi::c_int);
                    if operands.is_none() {
                        format_log1(
                            &mut *es,
                            c"format_replace_expression".as_ptr(),
...
unsafe fn format_replace(
    ...
    unsafe {
        ...
        copy = copy0.as_ptr();
        let list = format_build_modifiers(&mut *es, &mut copy).unwrap_or_default();
                ...
                match fm.modifier[0 as ::core::ffi::c_int as usize] as ::core::ffi::c_int {
                    ...
                    116 => {
                        ...
                        if !((fm.argv.len() as ::core::ffi::c_int) < 1 as ::core::ffi::c_int) {
                            ...
                            {
                                time_format = Some(format_strip(
                                    &mut *es,
                                    CStr::from_ptr(fm.argv[1].as_ptr()),
                                ));
                            }
                        }
                    }
        ...
        if modifiers & FORMAT_LITERAL != 0 {
            ...
            value = Some(format_unescape(&mut *es, CStr::from_ptr(copy)));
        } else if modifiers & FORMAT_CHARACTER != 0 {
            let new = format_expand1(&mut *es, CStr::from_ptr(copy));
            ...
            if modifiers & FORMAT_SESSIONS != 0 {
                value = format_loop_sessions(&mut *es, CStr::from_ptr(copy), &sort_crit);
                ...
                value = format_loop_windows(&mut *es, CStr::from_ptr(copy), &sort_crit);
                current_block = if value.is_some() {
                    4781510679662115254
                } else {
                    75153483021275631
                ...
                value = format_loop_panes(&mut *es, CStr::from_ptr(copy), &sort_crit);
                current_block = if value.is_some() {
                    4781510679662115254
                } else {
                    75153483021275631
                ...
                value = format_loop_clients(&mut *es, CStr::from_ptr(copy), &sort_crit);
                current_block = if value.is_some() {
                    4781510679662115254
                } else {
                    75153483021275631
                ...
                value = format_window_name(&mut *es, CStr::from_ptr(copy));
                current_block = if value.is_some() {
                    4781510679662115254
                } else {
                    75153483021275631
                ...
                value = format_session_name(&mut *es, CStr::from_ptr(copy));
                current_block = if value.is_some() {
                    4781510679662115254
                } else {
                    75153483021275631
                ...
                if let Some((left, right)) = format_choose(&mut *es, copy, 1 as ::core::ffi::c_int)
                {
                    let parsed = strtonum(
                        right.as_ptr(),
                ...
                if *copy as ::core::ffi::c_int == '?' as i32 {
                    ...
                    loop {
                        cp2 = format_skip1(Some(&mut *es), cp, c",");
                        if cp2.is_null() {
                            format_log1(
                                &mut *es,
                                c"format_replace".as_ptr(),
                            ...
                            value = Some(format_expand1(&mut *es, CStr::from_ptr(cp)));
                            break;
                        } else {
                            ...
                            if found.is_none() {
                                let expanded = format_expand1(&mut *es, &condition);
                            ...
                            if format_true(found.as_deref()) != 0 {
                                ...
                                if cp2.is_null() {
                                    ...
                                    value = Some(format_expand1(&mut *es, &right));
                                }
                                break;
                            } else {
                                format_log1(
                    ...
                    value = format_replace_expression(mexp, &mut *es, CStr::from_ptr(copy));
                    if value.is_none() {
                        value = Some(CString::default());
                    }
                    ...
                    value = Some(format_expand1(&mut *es, CStr::from_ptr(copy)));
                } else {
                    let time_format_ptr = time_format.as_deref();
                    if let Some(result) =
        ...
        if modifiers & FORMAT_EXPAND != 0 {
            value = format_expand1(&mut *es, &value);
        ...
        i = 0 as u_int;
        while i < nsub {
            let left = format_expand1(&mut *es, CStr::from_ptr(sub[i as usize].argv[0].as_ptr()));
            let right = format_expand1(&mut *es, CStr::from_ptr(sub[i as usize].argv[1].as_ptr()));
                ...
                match ch {
                    40 => {
                            ...
                            || es.flags & FORMAT_EXPAND_NOJOBS != 0
                        {
                            ...
                            let out = format_job_get(&mut *es, &name);
                            format_log1(
                                &mut *es,
                                c"format_expand1".as_ptr(),
                                c"#() result: %s".as_ptr(),
                    ...
                    123 => {
                        ...
                        if format_replace(&mut *es, fmt, n, &mut buf) != 0 as ::core::ffi::c_int {
                            break;
                        }
                        fmt = fmt.add(n.wrapping_add(1 as size_t));
                        continue;
                    ...
                    _ => {
                        ...
                        if s.is_null() {
                            ...
                            if format_replace(&mut *es, s, n, &mut buf) != 0 as ::core::ffi::c_int {
                                break;
                            } else {
                                continue;
                            }
...
pub unsafe fn format_expand_time(ft: &mut format_tree, fmt: &CStr) -> CString {
    unsafe {
        let mut es = format_expand_state::default();
        es.ft = ft;
...
pub unsafe fn format_defaults(
    ...
    unsafe {
        ...
        if !c.is_null() && (*c).name.is_some() {
            log_debug(
                c"%s: c=%s".as_ptr(),
                fmt_args![c"format_defaults".as_ptr(), cstr_ptr(&(*c).name)],
            );
        } else {
            log_debug(
                c"%s: c=none".as_ptr(),
        ...
        if !c.is_null() {
            format_defaults_client(&mut *ft, c);
        }
        if !s.is_null() {
            format_defaults_session(&mut *ft, s);
        }
        if !wl.is_null() {
            format_defaults_winlink(&mut *ft, wl);
        }
        if !wp.is_null() {
            format_defaults_pane(&mut *ft, wp);
        }
...
unsafe fn format_defaults_winlink(ft: &mut format_tree, mut wl: *mut winlink) {
    unsafe {
        if (*ft).window().is_null() {
            format_defaults_window(&mut *ft, (*wl).window());
        }
        (*ft).set_winlink(wl);
    }
}
pub unsafe fn format_defaults_pane(ft: &mut format_tree, mut wp: *mut window_pane) {
    unsafe {
        let mut wme: *mut window_mode_entry = ::core::ptr::null_mut::<window_mode_entry>();
        if (*ft).window().is_null() {
            format_defaults_window(&mut *ft, (*wp).window);
        }
        (*ft).set_pane(wp);
...
unsafe fn format_is_word_separator(ws: &CStr, mut gc: *const grid_cell) -> ::core::ffi::c_int {
    unsafe {
...
pub unsafe fn format_grid_word(mut gd: *mut grid, mut x: u_int, mut y: u_int) -> Option<CString> {
    unsafe {
        ...
        loop {
            ...
            if !(gc.flags as ::core::ffi::c_int) & GRID_FLAG_PADDING != 0
                && format_is_word_separator(CStr::from_ptr(ws), &raw mut gc) != 0
            {
                found = 1 as ::core::ffi::c_int;
                break;
            } else {
        ...
        loop {
            ...
            if format_is_word_separator(CStr::from_ptr(ws), &raw mut gc) != 0 {
                break;
            }
            ud.push(gc.data);
        }
