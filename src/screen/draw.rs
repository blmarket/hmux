use super::Screen;
use super::write::{
    screen_write_cell, screen_write_clearendofline, screen_write_ctx, screen_write_cursormove,
    screen_write_fast_copy, screen_write_putc, screen_write_start, screen_write_state,
    screen_write_stop,
};
use crate::fmt_args;
...
pub use crate::consts::{
    ...
    STYLE_LIST_LEFT_MARKER, STYLE_LIST_OFF, STYLE_LIST_ON, STYLE_RANGE_LEFT, STYLE_RANGE_NONE,
...
unsafe fn format_draw_put_list(
    ...
    unsafe {
        if width >= screens[LIST].cursor().0 {
        ...
        if start.wrapping_add(width) > screens[LIST].cursor().0 {
            start = screens[LIST].cursor().0.wrapping_sub(width);
        }

        let marker_left = screens[LIST_LEFT].cursor().0;
        let marker_right = screens[LIST_RIGHT].cursor().0;
        ...
        if start.wrapping_add(width) < screens[LIST].cursor().0 && width > marker_right {
...
unsafe fn format_draw_none(
    ...
    unsafe {
        let mut width_left = screens[LEFT].cursor().0;
        let mut width_centre = screens[CENTRE].cursor().0;
        let mut width_right = screens[RIGHT].cursor().0;
        let mut width_abs_centre = screens[ABSOLUTE_CENTRE].cursor().0;
        ...
        format_draw_put(
            ...
            screens[RIGHT].cursor().0.wrapping_sub(width_right),
            width_right,
        );
        format_draw_put(
            octx,
            ...
            (screens[CENTRE].cursor().0 / 2).wrapping_sub(width_centre / 2),
            width_centre,
        );
...
unsafe fn format_draw_left(
    ...
    unsafe {
        ...
        let mut width_right = screens[RIGHT].cursor().0;
        let mut width_abs_centre = screens[ABSOLUTE_CENTRE].cursor().0;
        let mut width_list = screens[LIST].cursor().0;
        let mut width_after = screens[AFTER].cursor().0;
        ...
        if width_list == 0 {
            let (aligned, after) = screens.split_at_mut(AFTER);
            let mut state = screen_write_start(&mut aligned[LEFT]);
            let mut ctx = screen_write_ctx::new(&mut state, &mut aligned[LEFT]);
            screen_write_fast_copy(&mut ctx, &after[0], 0, 0, width_after, 1);
...
unsafe fn format_draw_centre(
    ...
    unsafe {
        ...
        if width_list == 0 {
            let (aligned, after) = screens.split_at_mut(AFTER);
            let mut state = screen_write_start(&mut aligned[CENTRE]);
            let mut ctx = screen_write_ctx::new(&mut state, &mut aligned[CENTRE]);
            screen_write_fast_copy(&mut ctx, &after[0], 0, 0, width_after, 1);
            screen_write_stop(&mut ctx);
            drop(ctx);
            format_draw_none(octx, available, ocx, ocy, screens, frs);
        ...
        if focus_start == -1 || focus_end == -1 {
            focus_end = (screens[LIST].cursor().0 / 2) as c_int;
...
unsafe fn format_draw_right(
    ...
    unsafe {
        ...
        if width_list == 0 {
            let (aligned, after) = screens.split_at_mut(AFTER);
            let mut state = screen_write_start(&mut aligned[RIGHT]);
            let mut ctx = screen_write_ctx::new(&mut state, &mut aligned[RIGHT]);
            screen_write_fast_copy(&mut ctx, &after[0], 0, 0, width_after, 1);
            screen_write_stop(&mut ctx);
            drop(ctx);
            format_draw_none(octx, available, ocx, ocy, screens, frs);
        ...
        format_draw_put(
            ...
            screens[AFTER].cursor().0.wrapping_sub(width_after),
            width_after,
        );
...
pub(super) unsafe fn format_draw(
    octx: &mut screen_write_ctx,
    base: &grid_cell,
    available: u_int,
    expanded: &[u8],
    ...
    unsafe {
        ...
        let bytes = expanded;
        let (ocx, ocy) = octx.screen().cursor();
        ...
        RustStyleCodec.set(&mut sy, &current_default);
         ...
         */
        let mut s: [RustScreen; TOTAL] = core::array::from_fn(|_| {
            RustScreen::new_with_server_options(bytes.len() as u_int, 1, 0)
        });
        let mut states: [screen_write_state; TOTAL] =
            core::array::from_fn(|i| screen_write_start(&mut s[i]));
        let mut targets = states.iter_mut().zip(s.iter_mut());
        let mut ctx: [screen_write_ctx; TOTAL] = core::array::from_fn(|_| {
            let (state, screen) = targets.next().expect("each format screen has a state");
            screen_write_ctx::new(state, screen)
        });
        let mut width: [u_int; TOTAL] = [0; TOTAL];
        for ctx in &mut ctx {
            screen_write_clearendofline(ctx, current_default.bg as u_int);
        }
        ...
        while at < bytes.len() {
            ...
            if bytes[at] == b'#' && bytes.get(at + 1) != Some(&b'[') && at + 1 < bytes.len() {
                ...
                if even {
                    ...
                    screen_write_cell(&mut ctx[current], &sy.gc);
                    width[current] += 1;
                }
                continue;
            }
            ...
            RustStyleCodec.copy(&mut saved_sy, &sy);
            if RustStyleCodec
                .parse(&mut sy, &current_default, text)
                .is_err()
            {
                log_debug(
                    c"%s: invalid style '%.*s'",
            ...
            log_debug(
                ...
                fmt_args![
                    ...
                    text.len() as c_int,
                    text.as_ptr(),
                    RustStyleCodec.to_string(&sy).as_c_str()
                ],
            );
            ...
            match sy.list {
                ...
                STYLE_LIST_ON => {
                    ...
                    if focus_start != -1 && focus_end == -1 {
                        focus_end = ctx[LIST].screen().cursor().0 as c_int;
                    }
                    current = LIST;
                }
                /* Entering the focus. */
                STYLE_LIST_FOCUS => {
                    if list_state == 0 && focus_start == -1 {
                        focus_start = ctx[LIST].screen().cursor().0 as c_int;
                    }
                }
                ...
                _ => {
                    ...
                    if list_state == 0 && ctx[marker].screen().cursor().0 == 0 {
                        fr = None;
                        if focus_start != -1 && focus_end == -1 {
                            focus_end = -1;
                            focus_start = -1;
            ...
            if srs.is_some() {
                    ...
                    && !format_is_type(open, &sy)
                {
                    ...
                    if ctx[current].screen().cursor().0 != open.start {
                        open.end = ctx[current].screen().cursor().0.wrapping_add(1);
                        frs.push(open);
                    }
                }
                if fr.is_none() && sy.range_type != STYLE_RANGE_NONE {
                    fr = Some(format_range {
                        index: current,
                        start: ctx[current].screen().cursor().0,
                        end: 0,
...
pub(crate) fn format_width_impl(expanded: &[u8]) -> u_int {
    unsafe {
        let bytes = expanded;
...
pub(crate) fn format_trim_left_impl(expanded: &[u8], limit: u_int) -> CString {
    unsafe {
        let bytes = expanded;
        let mut ud = utf8_data::default();
...
pub(crate) fn format_trim_right_impl(expanded: &[u8], limit: u_int) -> CString {
    unsafe {
        let total_width = format_width_impl(expanded);
...
pub fn format_width(expanded: &[u8]) -> u_int {
    crate::FormatText::width(&crate::RustFormatText, expanded)
}

pub fn format_trim_left(expanded: &[u8], limit: u_int) -> CString {
    crate::FormatText::trim_left(&crate::RustFormatText, expanded, limit)
}

pub fn format_trim_right(expanded: &[u8], limit: u_int) -> CString {
    crate::FormatText::trim_right(&crate::RustFormatText, expanded, limit)
}
#[cfg(test)]
