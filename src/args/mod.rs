...
/// The opaque parsed arguments used by hmux.
///
/// ```compile_fail
/// use tmux_c2rs::RustArguments;
/// let args = RustArguments::from_strings(&[c"value"]);
/// let _ = args.tree;
/// ```
pub type RustArguments = args;
...
impl RustArguments {
    ...
    pub unsafe fn copy_with_arguments(&self, argv: &[CString]) -> Box<Self> {
        unsafe {
            ...
            new_args.count = self.count;
            new_args.values.reserve(self.values.len());
...
fn value_at(values: &[ArgsValue], i: usize) -> Option<&ArgsValue> {
    values.get(i)
}
...
unsafe fn args_value_as_string(value: &ArgsValue) -> &CStr {
    unsafe {
        match value {
            ArgsValue::None => c"",
        ...
        while i < values.len() {
            ...
            log_debug(
                ...
                fmt_args![
                    ...
                    args_value_type_to_string(value)
                ],
            );
            ...
            match type_0 {
                ...
                ARGS_PARSE_COMMANDS => {
                    if !matches!(value, ArgsValue::Commands { .. }) {
                        *cause = Some(xasprintf(
...
unsafe fn args_copy_copy_value(to: &mut ArgsValue, from: &ArgsValue, argv: &[CString]) {
    unsafe {
        *to = match from {
...
unsafe fn args_print_add_value(out: &mut Vec<u8>, value: &ArgsValue) {
    unsafe {
        if !out.is_empty() {
            out.push(b' ');
        }
