use super::widget::{
    ...
    mode_tree_start, mode_tree_zoom,
};
...
use crate::server::{client_ref_from_ptr, server_client_detach, server_client_suspend};
use crate::session::session_get_curw;
use crate::sort::sort_get_clients;
...
pub struct window_client_modedata {
    ...
    pub item_list: Vec<Box<window_client_itemdata>>,
...
impl window_client_itemdata {
    /// The client the row stands for, or null before one has been put in it.
...
pub static WINDOW_CLIENT_DEFAULT_FORMAT: &::core::ffi::CStr =
    c"#{t/p:client_activity}: session #{session_name}";
...
unsafe fn window_client_add_item(
    ...
    unsafe {
        ...
        items.push(Box::new(window_client_itemdata { client_ref: None }));
...
unsafe fn window_client_build(
    ...
    unsafe {
        let mut data: *mut window_client_modedata = modedata.client();
        let mut item: *mut window_client_itemdata =
            ::core::ptr::null_mut::<window_client_itemdata>();
        let mut i: u_int = 0;
        let mut c: *mut client = ::core::ptr::null_mut::<client>();
        (*data).item_list.clear();
        let l = sort_get_clients(sort_crit);
        for client_ptr in l {
            if !((*client_ptr).session.is_null()
                || (*client_ptr).flags & CLIENT_UNATTACHEDFLAGS as uint64_t != 0)
            {
                item = window_client_add_item(data);
                (*item).client_ref = client_ref_from_ptr(client_ptr);
            }
        }
        let mut current_block_20: u64;
        i = 0 as u_int;
        while (i as usize) < (*data).item_list.len() {
            item = &mut *(&mut (*data).item_list)[i as usize] as *mut window_client_itemdata;
            c = (*item).client();
            if !filter.is_null() {
            ...
            if current_block_20 == 4808432441040389987 {
                ...
                mode_tree_add(
                    ...
                    ModeTreeItemData::Client(item),
                    c as uint64_t,
                    &(*c).name.clone().expect("a name"),
                    Some(&text),
                    -(1 as ::core::ffi::c_int),
                );
            }
...
unsafe fn window_client_draw(
    ...
    unsafe {
        let mut item: *mut window_client_itemdata = itemdata.client();
        ...
        wp = window_get_active((*session_get_curw((*c).session)).window());
...
unsafe fn window_client_menu(mut modedata: WindowModeData, mut c: *mut client, mut key: key_code) {
    unsafe {
        let mut data: *mut window_client_modedata = modedata.client();
        ...
        if wme.is_null()
            || !matches!((*wme).state, WindowModeState::Client(_))
            || (*wme).state.client() != data
        {
            return;
        }
        window_client_key(
...
static window_client_help_lines: [&::core::ffi::CStr; 8] = [
...
pub(crate) unsafe fn window_client_free(wme: &mut window_mode_entry) {
    unsafe {
        let mut data: *mut window_client_modedata = wme.state.client();
        if data.is_null() {
            return;
        }
...
pub(crate) unsafe fn window_client_resize(
    wme: &mut window_mode_entry,
    mut sx: u_int,
    mut sy: u_int,
) {
...
pub(crate) unsafe fn window_client_update(wme: &mut window_mode_entry) {
    unsafe {
        let mut data: *mut window_client_modedata = wme.state.client();
...
unsafe fn window_client_do_detach(
    ...
    unsafe {
        let mut data: *mut window_client_modedata = modedata.client();
        let mut item: *mut window_client_itemdata = itemdata.client();
        if item == mode_tree_get_current(&(*data).tree_ref()).client() {
            mode_tree_down(&(*data).tree_ref(), 0 as ::core::ffi::c_int);
        }
...
pub(crate) unsafe fn window_client_key(
    wme: &mut window_mode_entry,
    mut c: *mut client,
    _s: *mut session,
    _wl: *mut winlink,
