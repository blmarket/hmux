pub use crate::consts::{UCHAR_MAX, VIS_CSTYLE, VIS_DQ, VIS_NL, VIS_NOSLASH, VIS_OCTAL, VIS_TAB};
pub use crate::types::*;
...
/// Tests graphical characters using the C library's current locale.
fn is_graph(b: u8) -> bool {
    unsafe { libc::isgraph(b.into()) != 0 }
}

/// Tests control characters using the C library's current locale.
fn is_control(b: u8) -> bool {
    unsafe { libc::iscntrl(b.into()) != 0 }
}
...
fn is_visible(c: core::ffi::c_int, flag: core::ffi::c_int) -> bool {
    ...
    printable_ascii
        ...
        || (flag & VIS_NL == 0 && c == '\n' as core::ffi::c_int)
        || (flag & VIS_SAFE != 0
            && (c == 0x08 || c == 0x07 || c == '\r' as core::ffi::c_int || is_graph(b)))
}
...
fn encode_bounded(src: &[u8], siz: usize, flag: core::ffi::c_int) -> (Vec<u8>, core::ffi::c_int) {
...
pub fn vis(c: core::ffi::c_int, flag: core::ffi::c_int, nextc: core::ffi::c_int) -> Vec<u8> {
    escape(c, flag, nextc).bytes().to_vec()
}

#[cfg(test)]
pub fn strvis(src: &core::ffi::CStr, flag: core::ffi::c_int) -> Vec<u8> {
    encode(src.to_bytes(), flag)
}

pub struct StrnvisResult {
    pub output: Vec<u8>,
    pub len: core::ffi::c_int,
}

pub fn strnvis(src: &core::ffi::CStr, siz: size_t, flag: core::ffi::c_int) -> StrnvisResult {
    let (out, len) = encode_bounded(src.to_bytes(), siz, flag);
    StrnvisResult { output: out, len }
}
...
pub fn stravis(src: &core::ffi::CStr, flag: core::ffi::c_int) -> std::ffi::CString {
    let out = encode(src.to_bytes(), flag);
    std::ffi::CString::new(out).expect("an encoded string holds no nul")
}

pub fn strvisx(src: &[u8], flag: core::ffi::c_int) -> Vec<u8> {
    encode(src, flag)
}
