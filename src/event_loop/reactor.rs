...
///
/// The caller owns every registered descriptor and must deregister it before
/// closing it. `poll` appends readiness notifications to `output`.
    ...
    R: Clone,
{
    ...
    fn reregister(&mut self, token: Token, interest: Interest) -> io::Result<()> {
        let raw_fd = self.registration(token)?.fd;
        let mut source = SourceFd(&raw_fd);
