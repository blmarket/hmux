...
use crate::options::{
    options_array_first, options_array_item_index, options_array_item_value, options_array_next,
    options_get_number,
};
use crate::paste::paste_add;
use crate::reactor::Timer;
use crate::server::server_client_handle_key;
use crate::session::session_get_curw;
use crate::session::session_theme_changed;
use crate::style::{colour_parseX11, colour_tostring};
use crate::terminfo::{tty_add_features, tty_default_features};
...
pub use crate::types::*;
use crate::window::window_update_focus;
...
unsafe fn tty_keys_add(mut tty: *mut tty, mut s: *const ::core::ffi::c_char, mut key: key_code) {
    unsafe {
        ...
        let bytes = ::core::slice::from_raw_parts(s as *const u8, strlen(s) as usize);
        match tty_keys_find(tty, bytes) {
            None => {
                log_debug(
                    c"new key %s: 0x%llx (%s)".as_ptr(),
                    fmt_args![s, key, keystr],
                );
...
unsafe fn tty_keys_add1(
    ...
    unsafe {
        ...
        if *s as ::core::ffi::c_int == tk.ch as ::core::ffi::c_int {
            ...
            tty_keys_add1(&mut tk.next, s, key);
        } else if (*s as ::core::ffi::c_int) < tk.ch as ::core::ffi::c_int {
            tty_keys_add1(&mut tk.left, s, key);
        } else if *s as ::core::ffi::c_int > tk.ch as ::core::ffi::c_int {
            tty_keys_add1(&mut tk.right, s, key);
        }
    }
}
pub unsafe fn tty_keys_build(mut tty: *mut tty) {
    unsafe {
        ...
        {
            tdkx = tty_default_xterm_keys.as_ptr().offset(i as isize);
            j = 2 as u_int;
            while (j as usize)
        ...
        o = options_get_ptr(global_options, c"user-keys".as_ptr());
        if !o.is_null() {
...
unsafe fn tty_keys_next1(
    ...
    unsafe {
        ...
        *size = 0 as size_t;
        let found = tty_keys_find(tty, ::core::slice::from_raw_parts(buf as *const u8, len))
    ...
    unsafe {
        ...
        *size = 0 as size_t;
        ...
        ::core::ptr::copy_nonoverlapping(
            buf.offset(2 as ::core::ffi::c_int as isize) as *const u8,
...
pub unsafe fn tty_keys_next(mut tty: *mut tty) -> ::core::ffi::c_int {
    unsafe {
        ...
        buf = (*tty).in_0.as_mut().unwrap().as_slice().as_ptr() as *const ::core::ffi::c_char;
        len = (*tty).in_0.as_ref().unwrap().len();
        if len == 0 as size_t {
            return 0 as ::core::ffi::c_int;
        }
        log_debug(
                    ...
                    if key != KEYC_UNKNOWN as ::core::ffi::c_ulong as key_code {
                        let event = Box::new(key_event {
                            ...
                            buf: if size == 0 {
                                ...
                                ::core::slice::from_raw_parts(buf as *const u8, size).to_vec()
                            },
                        });
                        server_client_handle_key(c, event);
                    }
                    (*tty).in_0.as_mut().unwrap().drain(size);
                    return 1 as ::core::ffi::c_int;
                }
                _ => {
                    log_debug(
                    ...
                    if (*tty).flags & TTY_TIMER != 0 {
                        ...
                        delay = options_get_number(global_options, c"escape-time".as_ptr())
                            as ::core::ffi::c_int;
...
unsafe fn tty_keys_extended_key(
    ...
    unsafe {
        ...
        *size = end.wrapping_add(1 as size_t);
        ...
        if nkey as ::core::ffi::c_ulonglong & KEYC_MASK_KEY
            == '\t' as i32 as ::core::ffi::c_ulonglong
        ...
        if (onlykey > 0x20 as key_code && onlykey < 0x7f as key_code
            || nkey as ::core::ffi::c_ulonglong & KEYC_MASK_TYPE
                ...
                && nkey as ::core::ffi::c_ulonglong & KEYC_MASK_KEY
...
unsafe fn tty_keys_clipboard(
    ...
    unsafe {
        let len = buf.len();
        let mut buf = buf.as_ptr() as *const ::core::ffi::c_char;
        let mut c: *mut client = tty_client(tty);
        let mut end: size_t = 0;
        ...
        *size = end.wrapping_add(1 as size_t);
        ...
        let mut copy = ::core::slice::from_raw_parts(buf as *const u8, end as usize).to_vec();
        copy.push(b'\0');
        needed = end
        ...
        log_debug(
            c"%s: %.*s".as_ptr(),
            fmt_args![c"tty_keys_clipboard".as_ptr(), outlen, out.as_ptr()],
        );
        input_request_reply(
            c,
    ...
    unsafe {
        ...
        *size = 0 as size_t;
        ...
        while (i as usize) < ::core::mem::size_of::<[::core::ffi::c_char; 128]>() as usize {
            ...
            if *buf.offset((3 as u_int).wrapping_add(i) as isize) as ::core::ffi::c_int
                >= 'a' as i32
        ...
        *size = (6 as u_int).wrapping_add(i) as size_t;
            ...
            && *buf.offset(3 as ::core::ffi::c_int as isize) as ::core::ffi::c_int == '0' as i32
        {
            if !c.is_null() {
                ...
                log_debug(
                    c"fg is %s".as_ptr(),
                    fmt_args![colour_tostring(n).as_c_str()],
                );
            }
            *fg = n;
...
unsafe fn tty_keys_palette(mut tty: *mut tty, buf: &[u8], size: &mut size_t) -> ::core::ffi::c_int {
    unsafe {
        ...
        *size = (5 as u_int).wrapping_add(i) as size_t;
        ...
        let colour = colour_parseX11(endptr.offset(1 as ::core::ffi::c_int as isize));
